...
function SubscriptionCard({ sub, paying, onEdit, onDelete, onPay }) {
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            {isActive && <Badge className="bg-emerald-500/15 text-emerald-600 border-emerald-500/20"><CheckCircle className="w-3 h-3 mr-1" /> Activa</Badge>}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm text-muted-foreground mt-3">
            <div className="flex items-center gap-2"><Users className="w-4 h-4" /><span>{sub.max_pilots} pilotos máx</span></div>
            <div className="flex items-center gap-2"><span>Drones: {sub.max_drones} máx</span></div>
            <div className="flex items-center gap-2"><Clock className="w-4 h-4" /><span>${sub.monthly_price_cop.toLocaleString("es-CO")} COP/mes</span></div>
            {isActive && countdown && (
          ...
          {isExpired && <p className="text-destructive text-sm mt-2">Vencida desde {format(new Date(sub.paid_until), "PPP", { locale: es })}</p>}
        </div>
        <div className="flex gap-2 flex-wrap justify-end">
          {isActive ? (
            <>
              <Button variant="outline" size="sm" onClick={() => onEdit(sub)}><Pencil className="w-4 h-4" /></Button>
              <Button variant="ghost" size="sm" onClick={() => onDelete(sub.id)} className="text-destructive hover:text-destructive">
                <Trash2 className="w-4 h-4" />
              </Button>
            </>
          ) : (
            <Button size="sm" className="gap-2" onClick={() => onPay(sub)} disabled={paying === sub.id}>
              {paying === sub.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CreditCard className="w-4 h-4" />}
              Pagar
            </Button>
          )}
        </div>
...
export default function CompanySubscription() {
  ...
  const [form, setForm] = useState({ company_name: "", monthly_price_cop: 0, max_pilots: 10, max_drones: 20 });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["company-subscriptions", user?.email] }); closeForm(); },
  });
  ...
  const openEdit = (sub) => {
    setEditingId(sub.id);
    setForm({ company_name: sub.company_name, monthly_price_cop: sub.monthly_price_cop, max_pilots: sub.max_pilots, max_drones: sub.max_drones });
    setShowForm(true);
  };
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid gap-4">
        ...
        {subscriptions.map((sub) => (
          <SubscriptionCard
            ...
            paying={paying}
            onEdit={openEdit}
            onDelete={(id) => { if (confirm("¿Eliminar esta suscripción?")) deleteMutation.mutate(id); }}
            onPay={handlePay}
          />
        ))}
      </div>
      ...
      <Dialog open={showForm} onOpenChange={v => { if (!v) closeForm(); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editingId ? "Editar Suscripción" : "Nueva Suscripción Empresarial"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-medium">Nombre de la Empresa *</label>
              <Input value={form.company_name} onChange={e => setForm({ ...form, company_name: e.target.value })} placeholder="Ej: Drones Colombia SAS" required />
            </div>
            <div>
              <label className="text-sm font-medium">Precio Mensual (COP) *</label>
              <Input type="number" min="0" value={form.monthly_price_cop} onChange={e => setForm({ ...form, monthly_price_cop: e.target.value })} placeholder="0" required />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium">Máx. Pilotos</label>
                <Input type="number" min="1" value={form.max_pilots} onChange={e => setForm({ ...form, max_pilots: e.target.value })} />
              </div>
              <div>
                <label className="text-sm font-medium">Máx. Drones</label>
                <Input type="number" min="1" value={form.max_drones} onChange={e => setForm({ ...form, max_drones: e.target.value })} />
              </div>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={closeForm}>Cancelar</Button>
              <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700">{editingId ? "Actualizar" : "Crear"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
